...
def demo_harris_detection():
    ...
    fig.suptitle(
        "Harris Corner Detection on Different Patterns", fontsize=14, fontweight="bold"
    )
...
def demo_full_pipeline_checkerboard():
    ...
    sim = create_image_based_simulator(
        image_array=checkerboard, max_features=16, gain=0.5, displacement="medium"
    )
...
def demo_comparison_patterns():
    ...
    fig.suptitle(
        "Visual Servoing with Different Patterns", fontsize=16, fontweight="bold"
    )
    ...
    for idx, (name, pattern) in enumerate(patterns.items()):
        ...
        # Create simulator
        sim = create_image_based_simulator(
            image_array=pattern, max_features=12, gain=0.5, displacement="medium"
        )
        ...
        ax_pattern.scatter(
            sim.image_scene.image_corners[:, 0],
    ...
    print("=" * 70)
    print(
        f"{'Pattern':<15} {'Features':<10} {'Converged':<12} {'Iterations':<12} {'Final Error':<15}"
    )
    print("-" * 70)

    for name, results in results_all.items():
        sim_name = name
        n_features = results_all[name][
            "iterations"
        ]  # This needs to be fixed, but for display
